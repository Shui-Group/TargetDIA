...
from .model import ion_model as lstm
from .model import similarity_calc as sim_calc
from .model import load_data as load_data
from .model import evaluate as evaluate
import numpy as np
...
with open(os.path.join(model_folder, 'log/test_%s.txt' % model_name),'w') as log_out:
    ...
    def test(folder, ce, ins, n, saveplot, phos = False):
        ...
        buckets = load_data.load_from_folder(folder, unmod_config, nce = ce, instrument = ins)
        ...
        if phos: 
            ...
            config = mod_config
            mod = '-mod'
        buckets = load_data.load_from_folder(folder, config, nce = ce, instrument = ins)
        print("[C] " + str(count_buckets(buckets)), file = log_out)
        output_buckets = pdeep.Predict(buckets)
        pcc, cos, spc, kdt, SA = sim_calc.CompareRNNPredict_buckets_tf(output_buckets, buckets)
        sim_names = ['PCC', 'COS', 'SPC', 'KDT', 'SA']
